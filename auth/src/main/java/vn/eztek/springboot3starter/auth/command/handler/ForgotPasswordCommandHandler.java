...
import vn.eztek.springboot3starter.auth.command.validator.ForgotPasswordCommandValidator;
...
public class ForgotPasswordCommandHandler implements
    CommandHandler<ForgotPasswordCommand, EmptyCommandResult, AuthAggregateId> {

  private final ForgotPasswordCommandValidator validator;
  ...
  @Override
  @Transactional
  public EmptyCommandResult handle(ForgotPasswordCommand command, AuthAggregateId authAggregateId) {
    // validating
    ...
    // event storing
    var event = UserForgotPasswordEvent.eventOf(authAggregateId, user.getId().toString(),
        user.getUsername());
    ...
    if (enableKeyEmulator) {
      redisMessagePublisher.publish(SendEmulatorMessage.create(List.of(
          SendEmulator.create(link, user.getUsername(), EventAction.FORGOT_PASSWORD.name(),
              keyRandom))));
    }
