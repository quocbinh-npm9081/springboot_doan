...
import vn.eztek.springboot3starter.task.command.validated.label.DeleteLabelCommandValidated;

@Component
@RequiredArgsConstructor
@Scope(value = "request", proxyMode = ScopedProxyMode.TARGET_CLASS)
public class DeleteLabelCommandValidator extends
        CommandValidation<DeleteLabelCommand, DeleteLabelCommandValidated> {
  ...
  @Override
  public DeleteLabelCommandValidated validate(DeleteLabelCommand command) {
    ...
    if (!isJoinedMember) {
      throw new AccessDeniedException("you-can-not-delete-label-in-this-task");
    }

    var label = labelRepository.findById(command.getId())
            .orElseThrow(() -> new NotFoundException("label-not-found"));
    return DeleteLabelCommandValidated.validatedOf(loggedInUser, label);
  }
}
