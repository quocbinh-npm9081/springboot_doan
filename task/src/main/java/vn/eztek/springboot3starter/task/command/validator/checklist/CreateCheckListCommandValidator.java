...
import vn.eztek.springboot3starter.task.command.validated.checklist.CreateCheckListCommandValidated;

@Component
@RequiredArgsConstructor
@Scope(value = "request", proxyMode = ScopedProxyMode.TARGET_CLASS)
public class CreateCheckListCommandValidator extends
    CommandValidation<CreateCheckListCommand, CreateCheckListCommandValidated> {
  ...
  @Override
  public CreateCheckListCommandValidated validate(CreateCheckListCommand command) {
    ...
    if (!isJoinedMember) {
      throw new AccessDeniedException("you-can-not-create-check-list-in-this-task");
    }

    return CreateCheckListCommandValidated.validatedOf(loggedInUser);
  }
