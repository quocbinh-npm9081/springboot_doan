...
import vn.eztek.springboot3starter.task.command.validated.checklistitem.UpdateDueDateCommandValidated;

@Component
@RequiredArgsConstructor
@Scope(value = "request", proxyMode = ScopedProxyMode.TARGET_CLASS)
public class UpdateDueDateCommandValidator extends
    CommandValidation<UpdateDueDateCommand, UpdateDueDateCommandValidated> {
  ...
  @Override
  public UpdateDueDateCommandValidated validate(UpdateDueDateCommand command) {
    ...
    ZonedDateTime thirtyMinutesLater = ZonedDateTime.now().plusMinutes(30);

    if (command.getDueDate().isBefore(thirtyMinutesLater)) {
      throw new BadRequestException("due-date-must-be-larger-30-minutes-from-now");
    }

    return UpdateDueDateCommandValidated.validatedOf(loggedInUser, checkListItem);
  }
