...
import vn.eztek.springboot3starter.task.command.validator.label.UpdateLabelCommandValidator;
...
public class UpdateLabelCommandHandler implements
        CommandHandler<UpdateLabelCommand, LabelResponse, TaskAggregateId> {
  private final UpdateLabelCommandValidator validator;
  ...
  @Override
  public LabelResponse handle(UpdateLabelCommand command, TaskAggregateId entityId) {
    // validating
    var validated = validator.validate(command);

    // handling
    var label = taskMapper.mapToLabelBeforeUpdate(validated.getLabel(),
            command.getName(), command.getColor());
    ...
    redisMessagePublisher.publish(
            SocketEventMessage.create(command.getTaskId().toString(), SocketEventType.TASK,
                    SocketForward.create(SocketResponseType.LABEL_UPDATED, gson.toJson(
