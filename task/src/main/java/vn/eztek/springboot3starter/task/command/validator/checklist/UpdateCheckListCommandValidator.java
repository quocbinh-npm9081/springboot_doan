...
import vn.eztek.springboot3starter.task.command.validated.checklist.UpdateCheckListCommandValidated;

@Component
@RequiredArgsConstructor
@Scope(value = "request", proxyMode = ScopedProxyMode.TARGET_CLASS)
public class UpdateCheckListCommandValidator extends
    CommandValidation<UpdateCheckListCommand, UpdateCheckListCommandValidated> {
  ...
  @Override
  public UpdateCheckListCommandValidated validate(UpdateCheckListCommand command) {
    ...
    if (!isJoinedMember) {
      throw new AccessDeniedException("you-can-not-update-check-list-in-this-task");
    }
    var checkList = checkListRepository.findById(command.getId())
        .orElseThrow(() -> new NotFoundException("check-list-not-found"));

    if (command.getName().equals(checkList.getName())) {
      throw new BadRequestException("check-list-name-is-already-exist");
    }

    return UpdateCheckListCommandValidated.validatedOf(loggedInUser, checkList);
  }
